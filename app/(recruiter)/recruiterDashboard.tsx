...
import { Briefcase, Building, CheckCircle, Clock, Users, XCircle } from 'lucide-react-native'
...
const RecruiterDashboard = () => {
    ...
    const studentsPerDay = (graphs?.studentsRegisteredPerDay || []).slice(0, 7).reverse()
    const studentsChartLabels = studentsPerDay.map((d: any) => {
        const date = new Date(d.date)
        return `${date.getDate()}/${date.getMonth() + 1}`
    })
    const studentsChartValues = studentsPerDay.map((d: any) => d.students)
            ...
            {isLoading ? (
                ...
                >
                    ...
                    <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: spacing.sm, marginBottom: spacing.sm }}>
                        <StatCard
                            icon={<Briefcase size={moderateScale(20)} color="#6C63FF" />}
                            label="Total Jobs"
                        ...
                        />
                        <StatCard
                            icon={<Users size={moderateScale(20)} color="#7C3AED" />}
                            label="Applicants"
                    ...
                    {studentsChartLabels.length > 0 && (
                        <View style={{ marginBottom: spacing.lg }}>
                            <Text style={{ ...typography.h4, marginBottom: spacing.sm }}>Students Registered (Last 7 Days)</Text>
                            <View
                                style={{
                                    borderRadius: spacing.sm,
                                    overflow: 'hidden',
                            ...
                            >
                                <LineChart
                                    data={{
                                        ...
                                        datasets: [{ data: studentsChartValues.length > 0 ? studentsChartValues : [0] }],
                                    }}
                                    width={SCREEN_WIDTH - spacing.md * 2 - 2}
                                    height={moderateScale(200)}
